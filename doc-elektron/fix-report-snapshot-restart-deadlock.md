...
Worth noting: there is no special-casing anywhere in this path by how many checkpoints were skipped. `MaybeActivateAutomaticSnapshot()`/`ActivateSnapshot()` run the exact same live in-place replacement whether the new snapshot is one checkpoint ahead of the old one or, as in this reproduction, several. A design that instead fell back to a fuller rebuild (closer to what `-reindex` does) once the gap exceeds some threshold would sidestep this whole class of failure for the case that matters most in practice -- a node that has been offline long enough to fall behind by multiple checkpoints is, by construction, the case where the live in-place swap has the most work to do and the largest window to be interrupted in.
