...
### 5.4 Windows is deliberately manual-only, and a second manual recovery option

**No automatic restart on Windows -- by decision, not oversight.** `util::ExecVp()` itself is actually already Windows-capable (its `#ifdef WIN32` branch converts to UTF-16 and calls `_wexecvp`), so a native Windows self-restart was evaluated: read the current process's argv via `GetCommandLineW()`/`CommandLineToArgvW()` (there is no `/proc/self/cmdline` equivalent, so this would have been genuinely new code, unlike everything else `RestartWithReindex()` uses), then call the already-Windows-capable `util::ExecVp()`. This was rejected: an automatic, process-replacing restart is exactly the kind of intervention that needs to be trustworthy, and this project's usual test environment has no Windows toolchain, so any Windows-side self-restart code could only ever be verified by code review, never by an actual run. Shipping an unverified restart-the-process code path was judged too risky relative to the benefit. Windows operators (and anyone else who reaches this point, as a documented fallback -- see below) instead get a clear, actionable log message and perform the recovery themselves.

**Every message an operator can reach at this point now names two concrete manual options**, built with exact absolute paths from the node's own datadir (`ManualDeleteHint()`, same anonymous-namespace helper file):

1. **Restart manually with `-reindex`.** Rescans the on-disk `blk*.dat` files without deleting them; wipes and rebuilds the block index and chainstate on top of what's found. Since mandatory pruning already means only a thin trailing window of recent blocks exists locally in this exact scenario, this doesn't meaningfully shortcut anything -- the node still ends up requesting a fresh snapshot from peers -- but it's the smaller, more familiar action for an operator already used to Bitcoin Core's `-reindex`.
2. **Stop the node and manually delete three directories together: `<datadir>/blocks`, `<datadir>/chainstate`, and `<datadir>/chainstate_snapshot`, then restart with no special flags.** This makes the node's on-disk state look exactly like a brand-new node's, which takes the separate, already-reliable "empty datadir" bootstrap path (`src/init.cpp`'s documented "3 special cases": empty datadir / `-reindex` / `-reindex-chainstate`, all handled the same way via `ImportBlocks()`'s trailing `ActivateBestChains()` call). **All three directories must be deleted together** -- deleting only `chainstate/` and `chainstate_snapshot/` while leaving `blocks/index/` (the block-index LevelDB) in place does *not* take this path: `chainman.m_best_header` stays populated straight from the surviving block index, so the node just re-enters the same automatic-restart logic instead (and, if it had already fired once, can immediately hit the one-shot guard's `FAILURE_FATAL`). The `snapshots/` directory (cached candidate `.dat` files) is harmless to leave behind either way. **Wallets and the configuration file are never part of this and must never be deleted** -- nothing in this recovery path reads or writes `wallets/`, `bitcoin.conf`, or `settings.json`.

Both options are now officially documented, supported recovery actions, not just implementation detail -- see the messages themselves (`ManualDeleteHint()`, the Windows-branch warning, the one-shot-guard `FAILURE_FATAL` text, and the POSIX early-return warnings) for the exact wording an operator will actually see.
