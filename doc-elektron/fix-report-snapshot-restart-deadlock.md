...
Unverified in this session whether `-reindex` (full) recovers a node stuck in this specific hung state, as opposed to the different corruption-detection failure in the companion report, which did recover via `-reindex`. That should be the first thing checked before recommending it as a workaround here.
