...
**Status:** Phase 1 (incremental accumulator), Phase 2 (snapshot metadata embedding), and Phase 3 (activation height gate) **implemented**, as of 2026-07-02, on branch `MuHash`. **Mainnet activated** at height `137000` (2026-07-02); testnet/testnet4/regtest remain at their existing low/placeholder heights for testing. Phase 4 (P2P protocol-version gate) remains deferred — see `doc-elektron/CHANGELOG-muhash-attestation.md` for the full implementation log, including two structural bugs found and fixed while implementing Phase 2. The analysis and recommendations below (§1–§8) are the original design record and are left as-is.
