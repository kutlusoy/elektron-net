...
- **See also:** [`guideline-wallet-integration.md`](./guideline-wallet-integration.md) §3.3 (Lightning Integration Options, Option A already chosen upstream in the wallet fork), [`guideline-electrs-fork-integration.md`](./guideline-electrs-fork-integration.md) (closest precedent for forking a third-party Bitcoin-adjacent server component for this chain; several findings below directly reuse its reasoning), [`guideline-lightning-node-deployment-lnd.md`](./guideline-lightning-node-deployment-lnd.md) (a parallel, separate plan for an LND gossip-routing node -- lighter resource footprint, cannot substitute for this document's trampoline-hub role, decided 2026-07-18 to be built *first*), [`BITCOIN_CORE_DIFF.md`](https://github.com/kutlusoy/elektron-net/blob/main/doc-elektron/BITCOIN_CORE_DIFF.md)
...
| BOLT11 HRP (Lightning invoices) | **Finalized (2026-07-18): `be`** (full invoice prefix `lnbe`), equal to `SEGWIT_HRP` -- see `elektron-net-electrum`'s `constants.py`/`doc/elektron.md` and `guideline-wallet-integration.md` SS3.1 | No longer blocking. BOLT11 spec defines the invoice HRP as `ln` + the chain's own bech32 address HRP; every established chain (Bitcoin, and by the same convention Litecoin) matches address HRP and invoice HRP exactly, so `be` is the correct value, not an arbitrary pick. This Eclair fork MUST use the same value -- it's now a known constant to implement against, not an open decision to make independently. |
...
- [x] BOLT11 HRP decision: finalized as `be` (SS3) -- implement against this fixed value, no longer open
...
1. ~~BOLT11 HRP: same open question already tracked in `guideline-wallet-integration.md` SS4/SS6.~~ **Resolved 2026-07-18:** `be`, see SS3.
