...
class AssumeutxoTest(BitcoinTestFramework):
    ...
    def test_restore_wallet_pruneheight(self, n3):
        self.log.info("Ensuring wallet can be restored from a backup that was created before the pruneheight via UTXO scan (pruned node)")
        ...
        # This backup (backup_w2.dat) was created at height 199, but balances are
        # recoverable from the current UTXO set on a pruned node (Elektron Net).
        n3.restorewallet("w2_pruneheight", "backup_w2.dat")
        w2_restored = n3.get_wallet_rpc("w2_pruneheight")
        assert_equal(w2_restored.getbalance(), 340)
